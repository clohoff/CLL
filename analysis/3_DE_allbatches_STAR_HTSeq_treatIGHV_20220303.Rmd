---
title: "DE and pathway enrichment analysis (treatment + IGHV) of all batches, pre-processed with STAR + HTSeq"
...
# Description
Old approach to assess treatment and IGHV status. 

In this differential gene expression analysis, we combine the information about treatment and IGHV status in one column, e.g. Ibrutinib_0 and Ibrutinib_1. This design leads to weird p-value histograms. 

Design formula: ~ 0 + patID + trIGHV
...
ddsAll <- ddsDrug[rowData(ddsDrug)$biotype == "protein_coding" & !rowData(ddsDrug)$symbol %in% c(NA, ""),
                  ddsDrug$diagnosis == "CLL" & ddsDrug$treatment != "Baseline" & ddsDrug$Replicate == 1 & ddsDrug$IGHV %in% c("U", "M")]
...
Add new column with treatment and IGHV status combined
```{r}
combined <- data.frame(colData(ddsAll)) %>%
  mutate(trIGHV = paste(ddsAll$treatment,ddsAll$IGHV,sep="_"))
combined$trIGHV[combined$treatment == "DMSO"] <- "DMSO" 

ddsAll$trIGHV <- combined[match(ddsAll$ID, combined$ID),]$trIGHV
```

## Create design matrix
```{r}
patAnno <- data.frame(colData(ddsAll)) %>% dplyr::select(patID, trIGHV) %>%
  mutate(trIGHV = str_replace_all(trIGHV,"[- ]","_")) %>%
  mutate(trIGHV = factor(trIGHV,
                         levels = c("DMSO",setdiff(unique(trIGHV),"DMSO"))))
designMat <- model.matrix(~ 0 + patID + trIGHV, patAnno)
```

Add design to dds objects
```{r}
ddsAll$condi <- factor(str_replace_all(ddsAll$trIGHV,"[- ]","_"))
...
# Save dds object
save(ddsAll, file = "./output/ddsAll_allbatches_STAR_HTSeq_treatIGHV_20220303.RData")
```
...
load("./output/ddsAll_allbatches_STAR_HTSeq_treatIGHV_20220303.RData")
DEresDis <- DESeq(ddsAll)
#ddsAllDis <- estimateDispersions(ddsAll)
#DEresDis <- nbinomWaldTest(ddsAllDis, maxit=500)
save(DEresDis, file = "./output/DEresDis_allbatches_STAR_HTSeq_treatIGHV_20220303.RData")
```

Get results
```{r}
load("./output/DEresDis_allbatches_STAR_HTSeq_treatIGHV_20220303.RData")
...
```{r prepare-camera, eval=FALSE, echo=FALSE}
...
Run camera function
```{r run-camera, eval=FALSE, echo=FALSE}
...
```{r run-camera-individually, eval=FALSE, echo=FALSE}
...
```{r plot-camera, fig.width=15, fig.height=13, eval=FALSE, echo=FALSE}
