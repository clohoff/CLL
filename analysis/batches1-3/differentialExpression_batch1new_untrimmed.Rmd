---
title: "Detect differential expressed genes of drug treatment of batch 1 new untrimmed"
...
Load data
```{r}
load("~/Documents/R/drugseq_test/output/ddsDrug_untrimmed_batch1new_20211125.RData")
...
```{r, eval=FALSE}
DEres <- DESeq(ddsAll)
save(DEres, file = "~/Documents/R/drugseq_test/output/DEres_batch1new_untrimmed_20211126.RData")
```

```{r}
load("~/Documents/R/drugseq_test/output/DEres_batch1new_untrimmed_20211126.RData")
```
...
# Gene list with all genes per treatment
geneList_all <- lapply(unique(allRes$treatment), function(tr) {
  filter(allRes, treatment == tr)$row
  })
names(geneList_all) <- unique(allRes$treatment)
...
#design <- matrix(1, nrow = 1, ncol = 2, dimnames = list("stat", c("Intercept","Group")))
pathways_H = gmt_to_csv("~/Documents/R/drugseq_test/data/h.all.v6.2.symbols.gmt.txt")
pathways_KEGG = gmt_to_csv("~/Documents/R/drugseq_test/data/c2.cp.kegg.v6.2.symbols.gmt.txt")
...
enrichPlot <- lapply(unique(allRes$treatment), function(tr) {
  ...
  res <- runGSAhyper(genes = geneList$tr, universe = geneList_all$tr, gsc = loadGSC(pathways_H))
...
enrichPlot <- lapply(unique(allRes$treatment), function(tr) {
  inputTab <- allRes %>% filter(pvalue <= 0.05, treatment == tr) %>%
    ...
    select(symbol, stat) %>%
    data.frame() %>% column_to_rownames("symbol")
  res <- jyluMisc::runGSEA(inputTab, gmts$KEGG, "page")                           # ToDo get package jyluMisc
  p <- plotEnrichmentBar(res, pCut =0.1, ifFDR= TRUE, setName = "Hallmark", 
                       title = tr, removePrefix = "HALLMARK_")
  p
})
