---
title: "DE and pathway enrichment analysis of batches 3 and 4, pre-processed with Salmon"
...
```
After filtering, more genes remain in the data set pre-processed with Salmon than with STAR + HTSeq. 
...
designMat <- model.matrix(~ 0 + patID + treatment, patAnno)
```
...
# Save dds object
save(ddsAll, file = "./output/ddsAll_batch34_Salmonu_20220221.RData")
```
...
save(DEresDis, file = "./output/DEresDis_batch34_Salmonu_20220221.RData")
```

Get results
```{r}
load("./output/DEresDis_batch34_Salmonu_20220221.RData")
...
# Filter and create new df
FDR <- allRes %>% filter(padj < 0.01, treatment == "Nutlin_3a") %>% select(row, symbol, log2FoldChange, pvalue, padj, treatment)
genes <- unique(FDR$row)

# Invariant filtering
RNAnorm_all$treatment <- factor(RNAnorm_all$treatment, levels = allTreat)
...
```
The result indicates that nearly all drugs effect the pathways cell adhesion molecules and type I diabetes mellitus. The rather few significant pathways identified after drug perturbation with C26 may be explained by the little number of significant DE genes (see bar plot above).
...
```{r enrich-cP-GSEA, fig.width=18, fig.height=12}
plot_list <- lapply(gseGO_results, function(tr) {
    if (nrow(tr@result)) {
...
```{r netplot-cP-GSEA, fig.width=10, fig.height=6}
library(ggnewscale)
