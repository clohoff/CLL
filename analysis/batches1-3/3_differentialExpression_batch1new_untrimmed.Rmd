...
# Load data and only use batch1new samples
```{r}
...
```{r}
gmts = list(H= "~/Documents/R/drugseq_test/data/h.all.v6.2.symbols.gmt.txt",
            KEGG = "~/Documents/R/drugseq_test/data/c2.cp.kegg.v6.2.symbols.gmt.txt")
design_camera <- data.frame(Group=c(0,1))

#limmaRes <- lapply(unique(allRes$treatment), function(tr) {
...
#    add_column(base = 0.0, .before = "stat")
#  limma::camera(inputTab, gmts$H, design_camera, inter.gene.cor=0.01)
#})
```
