...
allFiles <- list.files("~/Documents/drugSeq/bam/batch1_new/log/",recursive = TRUE, full.names = TRUE)
...
Annotate patients
```{r}
patAnno <- readxl::read_xlsx("~/Documents/R/drugseq_test/data/RNAseq_samples_all_batch1new.xlsx") %>%
...
all(patAnno$fileName %in% allRecord$filename)  # only batch 1 new, no pilot samples included
...
htTab <- read_tsv("~/Documents/R/drugseq_test/output/htseq_process_batch1new.tsv")
...
write_csv2(allRecordOut, file = "~/Documents/R/drugseq_test/output/STAR_HTseq_batch1new_summary.csv")
```

# Salmon results

```{r}
allDir <- list.dirs("~/Documents/drugSeq/salmon/pilot/", recursive = FALSE)  # pilot batch
...
```{r}
allDir <- list.dirs("~/Documents/drugSeq/salmon/batch1_new/", recursive = FALSE)  # batch 1 new
...
allRecordOut <- read_csv2("~/Documents/R/drugseq_test/output/STAR_HTseq_batch1new_summary.csv") %>%
...
resTab <- group_by(testTab, entry, metaVar) %>% nest() %>%
  ...
  mutate(res = map(m, broom::tidy)) %>%  # Problem: the standard deviation is zero
  unnest(res) %>%
