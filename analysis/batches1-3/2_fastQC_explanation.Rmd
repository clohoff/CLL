...
As output we get a report in interactive html format, several plots and a multiqc_data.json file (MultiQC: Summarize analysis results for multiple tools and samples in a single report, doi: 10.1093/bioinformatics/btw354).
...
Combine all batches in one large data frame
```{r combine_dfs}
df_all <- rbind(df_pilot, df_batch1, df_batch2, df_batch3)
```

# Visualize results

## Average sequence length
```{r fig-seqlength, fig.width=4, fig.height=3}
df_all %>%
  group_by(batch) %>%
  ...
  #scale_fill_brewer(palette="PRGn") # or "Accent", "Dark2", "Spectral"
```
...
## Sequence counts
We want to plot the sequence counts per sample. 
```{r fig-seqcounts, fig.width=20, fig.height=5}
df_all %>%
  ...
  scale_fill_manual(values=cp)
```
As expected, we observe a huge difference between batch 1 and all other batches. Batch 1 has a much higher read depth than the other batches. This is due to combining the old batch 1 with the new batch 1.

## Duplication rate
After plotting the sequence counts, we want to know how many reads are duplicates. Therefore, we visualize the duplication rate per sample for batch 3.
```{r fig-duplrate-batch3, fig.width=20, fig.height=5}
df_batch3 %>%
...
print(mean(df_batch3$general.percent_duplicates))
```
This plot shows that the duplication rate for batch 3 is between 30% and 80%, with a mean of 59%. The patient with the ID 708 has the lowest duplication rate. The Jurkat samples are the controls for each plate. The plot demonstrates that these controls are around the mean duplication rate. 


Visualize the duplication rate per sample for all batches ordered by patient ID.
```{r fig-duplrate-all, fig.width=20, fig.height=5}
df_all %>%
  ...
  scale_fill_manual(values=cp)
```
This figure illustrates that the sample IDs and therefore the patients are well stratified across the four different batches.

Visualize the duplication rate per sample for all batches ordered by batches.
```{r fig-duplrate-all2, fig.width=20, fig.height=5}
df_all %>%
  mutate(metadata.sample_id = fct_reorder(metadata.sample_id, batch)) %>%
  ggplot(aes(x=metadata.sample_id, y=general.percent_duplicates, fill=batch)) +
  geom_col() +
  labs(x="Sample ID", y="Percent duplicates") +
  ggtitle("Duplication rate ordered by batch") +
  theme(axis.text.x=element_blank()) +
  scale_fill_manual(values=cp)
```
There are some minor differences between the batches: The pilot batch has the highest duplication rate. The duplication rate of batch 1 is quite homogeneous in comparison to batches 2 and 3. 

## GC content

Visualize the per sequence GC content
```{r fig-gc, fig.width=20, fig.height=5}
df_all %>%
  dplyr::mutate(metadata.sample_id = forcats::fct_reorder(metadata.sample_id, batch)) %>%
  ggplot(aes(x=metadata.sample_id, y=general.percent_gc, fill=batch)) +
  geom_col() +
  labs(x="Sample ID", y="Percentage of GC") +
  ggtitle("Per sequence GC content") +
  theme(axis.text.x=element_blank()) +
  scale_fill_manual(values=cp)
```

```{r fif-gc-box, fig.width=5, fig.height=4}
df_all %>%
  group_by(batch) %>%
  ...
  scale_fill_manual(values=cp)
```
Again we see some differences between the batches. Batch 1 has the highest GC content (49.52%), followed by batch 2 and 3, whereas the pilot batch has a significantly lower mean GC value. For all batches the distribution of data is unsymmetric. The boxplot of batch 1 points from a population which is skewed to the left, whereas the populations of batches 2 and 3 are skewed to the right.


```{r gc-content, include=FALSE}
...
### Hypothesis Testing
Due to the differences in GC content, we test the hypothesis that the mean GC content of all batches is the same as the mean GC content in the human genome (41%). If we assume that GC content is normally distributed, we can do the following:
```{r gc-test}
...
```
As the p-values are very small for all batches, we reject the null hypothesis. The GC contents in the batches are significantly higher than usual. Thus, a *GC-content normalization approach* could be applied to avoid bias in the differential expression analysis. https://bmcbioinformatics.biomedcentral.com/articles/10.1186/1471-2105-12-480

## Failed sequence counts

```{r fig-seqfailed, fig.width=20, fig.height=5}
df_all %>%
  ggplot(aes(x=metadata.sample_id, y=general.percent_fails, fill=batch)) +
  geom_col() +
  labs(x="Sample ID", y="Percentage of failed reads") +
  ggtitle("Sequence reads considered as failed") +
  theme(axis.text.x=element_blank()) +
  scale_fill_manual(values=cp)
```
The percentage of failed sequence counts is lowest for the pilot batch.



# Extracting data from the plots

A big advantage of the MultiQC package is the possibility to directly extract data from the FastQC plots. 
In that way we can rebuild the plots from the FastQC html files while analyzing all batches.

## How To

Select specific columns
```{r extract-quality}
df_batch2_raw <- TidyMultiqc::load_multiqc("./batch2/multiqc_report_batch2_data/multiqc_data.json", sections = c('raw', 'general'))
df_batch2_raw %>% dplyr::select(contains('quality'))
```

Show the IDs of all plots
```{r extract-ids}
TidyMultiqc::list_plots("./batch2/multiqc_report_batch2_data/multiqc_data.json")
```

As an example, we can load the information from the Per Sequence Quality Scores plot.
```{r extract-per-sequence-quality}
per_seq_quality_scores_df_batch2 <- TidyMultiqc::load_multiqc(
  "./batch2/multiqc_report_batch2_data/multiqc_data.json", 
  sections = 'plot',
  plots = "fastqc_per_sequence_quality_scores_plot-1")
per_seq_quality_scores_df_batch2
```
We now extracted the plot data, but each sample has an entire data frame of plot data (see <tibble>). Thus, we first need to transform the data frames.

```{r extract-one-plot}
per_seq_quality_scores_df_batch2$`plot.fastqc_per_sequence_quality_scores_plot-1`[[1]]
```
Each data frame is a set of (x,y) pairs. In this case, the x value is the quality score, and y is the number of times that score has been counted. Therefore, the data is depicted in an histogram.

After transforming the data frame, we can plot the per sequence quality scores for one sample.
```{r fig-per-sequence-quality1, fig.width=6, fig.height=4}
df <- as.data.frame(per_seq_quality_scores_df_batch2$`plot.fastqc_per_sequence_quality_scores_plot-1`[[1]])
ggplot(df, aes(x=x, y=y)) +
  geom_line() +
  labs(x="Mean Sequence Quality (Phred Score)", y="Count") +
  ggtitle("Per sequence quality scores")
```

We can unnest the data frame to calculate summary statistics, e.g. the total number of reads. We can also get this information from the data frame df_all.
```{r unnest}
per_seq_quality_scores_df_batch2 %>%
  unnest(cols = `plot.fastqc_per_sequence_quality_scores_plot-1`) %>%
  group_by(metadata.sample_id) %>%
  summarise(total_reads = sum(y))
```
...
Using the purrr method, we can map each plot data frame into a row of summary statistics:
```{r histogram-summary}
df_summary <- per_seq_quality_scores_df_batch2 %>%
  dplyr::mutate(
...
We can show these summary statistics in a plot:
```{r fig-histogram-summary, fig.width=20, fig.height=5}
...
ggplot(df_summarylong,
  ...
  ggtitle("Sequence quality summary statistics") +
  theme(axis.text.x=element_blank()) +
  scale_fill_manual(values=cp)
```

# Plot results from FastQC summary html file
...
create_plotdf <- function(plotname, column, xtitle, ytitle, title) {
  ...
  path_batch_list <- list(c("batch1_combined/multiqc_report_batch1c_data/multiqc_data.json", "batch1"),
...
```{r plot-mean-quality-scores, fig.width=10, fig.height=4}
plot_mean_quality_scores <- create_plotdf(plotname = "fastqc_per_base_sequence_quality_plot-1",
                                                 column = "plot.fastqc_per_base_sequence_quality_plot-1",
                                                 xtitle = "Position in read (bp)",
                                                 ytitle = "Phred score", title = "Mean quality scores")
...
```{r plot-per-seq-quality-scores, fig.width=10, fig.height=4}
plot_per_sequence_quality_scores <- create_plotdf(plotname = "fastqc_per_sequence_quality_scores_plot-1",
                                                 column = "plot.fastqc_per_sequence_quality_scores_plot-1",
                                                 xtitle = "Mean Sequence Quality (Phred Score)",
                                                 ytitle = "Count", title = "Quality score distribution over all sequences")
...
**ToDo**: Import per sequence gc content plots directly from MultiQC and show the five plots next to each other.

This module measures the GC content across the whole length of each sequence in a file and compares it to a modeled normal distribution.
In a normal random library one would expect a roughly normal distribution of GC content where the central peak corresponds to the overall GC content of the underlying genome. An unusually shaped distribution could indicate a contaminated library or some other kinds of biased subset. A normal distribution which is shifted indicates some systematic bias which is independent of base position.
```{r plot-per-seq-gc-content, fig.width=10, fig.height=4}
#plot_per_sequence_gc_content <- create_plotdf(plotname = "fastqc_per_sequence_gc_content_plot-1",
#                                              column = "plot.fastqc_per_sequence_gc_content_plot-1",
#                                              xtitle = "% GC", ytitle = "Percentage",
#                                              title = "Per sequence GC content")
#print(plot_per_sequence_gc_content)
```
As we can see a distribution with two peaks, the library could be contaminated.
Perhaps adapter trimming/quality trimming helps or it is a specific contaminant which is often detected by the over-represented sequences plot.
...
```{r plot-per-base-n-content, fig.width=10, fig.height=4}
plot_per_base_n_content <- create_plotdf(plotname = "fastqc_per_base_n_content_plot-1",
                                                 column = "plot.fastqc_per_base_n_content_plot-1",
                                                 xtitle = "Position in read (bp)",
                                                 ytitle = "Percentage N-count", title = "Per base N content")
...
This module counts the degree of duplication for every sequence in a library and creates a plot showing the relative number of sequences with different degrees of duplication. The depicted line takes the full sequence set and shows how its duplication levels are distributed. In a properly diverse library most sequences should fall into the far left of the plot. A general level of enrichment, indicating broad over-sequencing in the library, will tend to flatten the lines, lowering the low end and generally raising other categories. More specific enrichment of subsets, or the presence of low complexity contaminants, will tend to produce spikes towards the right of the plot. 
...
#load_multiqc(
#    "./batch2/multiqc_report_batch2_data/multiqc_data.json",
#    sections='plot', plots="fastqc_sequence_duplication_levels_plot-1")
```

```{r plot-seq-dupl-level, fig.width=10, fig.height=4}
#plot_seq_duplication_level <- create_plotdf(plotname = "fastqc_sequence_duplication_levels_plot-1",
#                                                 column = "plot.fastqc_sequence_duplication_levels_plot-1",
#                                                 xtitle = "Sequence duplication level",
#                                                 ytitle = "% of library", title = "Sequence Duplication Levels")
#print(plot_seq_duplication_level)
```

In the plots we can observe peaks around a sequence duplication level of 10. For RNA-Seq data this is normal and not necessarily a hint of poor quality.
...
Explanations from https://www.bioinformatics.babraham.ac.uk/projects/fastqc/Help/3%20Analysis%20Modules/8%20Duplicate%20Sequences.html.
...
plot_adapter_content <- create_plotdf(plotname = "fastqc_adapter_content_plot-1",
...
```{r fig-over-rep-sequences, fig.width=20, fig.height=5}
overrep_seq_df %>%
  mutate(metadata.sample_id = fct_reorder(metadata.sample_id, batch)) %>%
