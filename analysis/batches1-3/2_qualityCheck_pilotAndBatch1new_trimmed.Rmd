---
title: "Quality assessment of drug perturbation RNAseq data (pilot and batch 1 new trimmed)"
...
### PCA colored by trisomy12
```{r}
ggplot(pcaTab, aes(x=PC1,y=PC2, col = trisomy12)) + geom_point() + theme_bw() + 
  xlab(sprintf("PC1 (%2.1f%%)",varExp[1])) + ylab(sprintf("PC2 (%2.1f%%)",varExp[2])) 
```
...
ddsSub <- ddsDrug[,ddsDrug$treatment %in% "Baseline"]  # 9 samples
overPat <- intersect(ddsCLL$patientID, ddsSub$patID)   # 5 overlapping patients
...
```
5 overlapping patient IDs between old and new RNAseq data processed with Salmon.
...
# Compare with previous RNAseq data (DMSO as baseline)

## Processing datasets
```{r}
load("~/Documents/R/drugseq_test/data/ddsCLL_Salmon_190702.RData")
ddsSub <- ddsDrug[,ddsDrug$treatment %in% "DMSO"]    # 34 samples were treated with DMSO
...
colnames(ddsNew) <- ddsNew$patID
```
31 patient IDs treated with DMSO are shared between old (2019) and new (2021) RNAseq data. 
...
colnames(ddsNew) <- ddsNew$patID
```
31 patient IDs treated with DMSO are shared between old (2019) and new batch 1 (2021) RNAseq data.
