---
title: "MOFA+ applied to scTransformed RNASeq data"
...
ddsDrug <- ddsDrug[, !ddsDrug$patID %in% remPat]
...
ddsAll <- ddsDrug[rowData(ddsDrug)$biotype == "protein_coding" &
                    !rowData(ddsDrug)$symbol %in% c(NA, ""),]
...
genes_df <- dplyr::select(genes_df, -patID, -batch)
genes_df <- column_to_rownames(genes_df, "ID")

genes_df_sub <- genes_df %>%
  dplyr::select(pretreated, gender, IGHV, trisomy12, NOTCH1, TP53, ATM,
...
ddsAll <- DESeqDataSetFromMatrix(countMat.norm,
                                     colData = colData(ddsAllbefore), design = ~1)
rowData(ddsAll) <- rowData(ddsAllbefore[rownames(countMat.norm)],)
...
save(MOFAobject_res, file = "./output/MOFAres_STAR_woGrouping_scTrans_20220618.RData")
# converged after 46 iterations (ELBO=-1921684.71, deltaELBO=188.467 (0.00020700%), Factors=5)
```
...
```{r}
load("./output/MOFAres_STAR_woGrouping_scTrans_20220618.RData")
...
save(MOFAobject_res, file = "./output/MOFAres_STAR_focusIbrDuv_scTrans_20220618.RData")
# converged after 76 iterations (ELBO=-407578.08, deltaELBO=43.886 (0.00024345%), Factors=5)
```

Load MOFA model
```{r}
load("./output/MOFAres_STAR_focusIbrDuv_scTrans_20220618.RData")
```
...
save(MOFAobject_res, file = "./output/MOFAres_STAR_groupingIbrDuvNutDMSO_scTrans_20220618.RData")
# converged after 76 iterations (LBO=-745020.92, deltaELBO=162.499 (0.00044749%), Factors=19)
```

Load MOFA model
```{r}
load("./output/MOFAres_STAR_groupingIbrDuvNutDMSO_scTrans_20220618.RData")
```
...
save(MOFAobject_res, file = "./output/MOFAres_STAR_groupingIbrDuvNutDMSO1view_scTrans_20220618.RData")
# converged after 76 iterations (ELBO=-766213.57, deltaELBO=148.675 (0.00041104%), Factors=15)
```

Load MOFA model
```{r}
load("./output/MOFAres_STAR_groupingIbrDuvNutDMSO1view_scTrans_20220618.RData")
```
...
save(MOFAobject_res, file = "./output/MOFAres_STAR_groupingIbrDuvNutDMSO2views_adjViab_scTrans_20220618.RData")
# converged after 41 iterations (ELBO=-89021.58, deltaELBO=62.979 (0.00018579%), Factors=12)
```

Load MOFA model
```{r}
load("./output/MOFAres_STAR_groupingIbrDuvNutDMSO2views_adjViab_scTrans_20220618.RData")
```
...
save(MOFAobject_res, file = "./output/MOFAres_STAR_viewingIbrDuvNutDMSO_scTrans_20220618.RData")
# converged after 166 iterations (ELBO=5211435.19, deltaELBO=21.476 (0.00006792%), Factors=9)
```

Load MOFA model
```{r}
load("./output/MOFAres_STAR_viewingIbrDuvNutDMSO_scTrans_20220618.RData")
```
...
save(MOFAobject_res, file = "./output/MOFAres_STAR_viewingIbrDuvNutDMSOwoViab_scTrans_20220618.RData")
# converged after 111 iterations (ELBO=-777131.93, deltaELBO=32.765 (0.00010553%), Factors=16)
```
...
Load MOFA model
```{r}
load("./output/MOFAres_STAR_viewingIbrDuvNutDMSOwoViab_scTrans_20220618.RData")
```
...
save(MOFAobject_res, file = "./output/MOFAres_STAR_viewingIbrDuvNutsubtractDMSO_scTrans_20220618.RData")
# converged after 999 iterations (ELBO=-637964.77, deltaELBO=24.868 (0.00015630%), Factors=14)
```
...
Load MOFA model
```{r}
load("./output/MOFAres_STAR_viewingIbrDuvNutsubtractDMSO_scTrans_20220618.RData")
```
